...
class ModelTrainer:
    ...
    def train(self):
        ...
        all_train_embeddings = self.precomputed_embeddings['train']
        all_val_embeddings = self.precomputed_embeddings['val']
        all_test_embeddings = self.precomputed_embeddings['test']
        all_knowledge_embeddings = self.precomputed_embeddings['knowledge']
    ...
    def calculate_hierarchy_accuracy(
        ...
        Args:
            pred_disease: 预测的疾病名称
            true_disease: 真实的疾病名称
            disease_to_parent: 疾病到父节点的映射字典
            parent_to_grandparent: 父节点到祖父节点的映射字典（可选）
            
        Returns:
            float:  (1.0 for exact match, 0.5 for same parent, 0.25 for same grandparent)
        """
